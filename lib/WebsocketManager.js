class WebsocketManager {
	...
	add(userId, key, ws, onData = null) {
		...
		ws.addEventListener('close', (code, reason) => {
			console.log("Removing socket", {code, reason});
			removeSocket();
		});
		...
		ws.addEventListener('message', e => {
			console.log("Received data", e.data, {userId, key, ws});
			if (onData) {
				onData(e.data);
			}
		});
		console.log("Added", {userId, key, ws});
	}
	/**
	 * Sends a message to all websockets belonging to a user
	 * that match a given `key`.
	...
	send(userId, key, data, options = {}) {
		...
		return new Promise((function (resolve, reject) {
			console.log(userId, key, data, this._map);
		...
		console.log("SENT", {userId, key, data});
	}
}
